...
async function initializeDatabase() {
  ...
  if (envContent.includes("your-secret-key-here-make-it-long-and-random")) {
    ...
    console.log(
      "💡 Please add your BETTER_AUTH_SECRET to .env before running database migration",
    );
    console.log(
  ...
  try {
    ...
    try {
      execSync(
        'docker ps --format "table {{.Names}}" | grep -E "(postgres|db)"',
        { encoding: "utf8", stdio: "pipe" },
      );
      console.log("✅ Database container is running");
    } catch (error) {
      console.log(
        "❌ Database container not found. Make sure Docker containers are running.",
      );
    ...
    console.log(
      "💡 This will create or update the required tables for authentication",
    );
    ...
    try {
      ...
      console.log(
        "   ✉️  verification - Email verification and password resets",
      );
      console.log("");
    ...
    console.log(
      "💡 You can run database migration manually later with: npm run db:migrate",
    );
  }
}
